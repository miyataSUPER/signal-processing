...
def design_highpass_filter(cutoff_freq, sampling_freq, filter_order):
    """
    ハイパスフィルタを設計する関数
    ...
    filter_coefficients = signal.firwin(
        ...
        normalized_cutoff,  # 正規化された遮断周波数
        window='hamming',   # 窓関数
        pass_zero=False     # ハイパスフィルタの指定
    )
...
def main():
    ...
    try:
        # フィルタを設計
        filter_coefficients = design_highpass_filter(
            Constants.CUTOFF_FREQ,
